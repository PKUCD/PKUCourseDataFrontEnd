<template>
<el-container>
  <el-header><div class="text" style="text-align:center;"><h2>修改页面</h2></div></el-header>
...
export default {
    ...
    data() {
      return {
        ...
        rules: {
          name: [
            { required: true, message: '请修改活动名称', trigger: 'blur' },
            { min: 0, max: 50, message: '长度50个字符以内', trigger: 'blur' }
          ],
          desc: [
            { required: true, message: '请修改活动形式', trigger: 'blur' }
          ]
        },
    ...
    handleClose(tag) {
      ...
      async GET(){
        ...
        const {ruleForm,descdynamicTags:res}=await this.$ajax.get(url);
        console.log(res)
      }
    }
  }
